...
±±ºPrograma  ³NCRepCrC	ºAutor  ³Elton C.			 º Data ³  05/11/11   º±±
...
Local cPrefixo  := U_MyNewSX6("NC_PREFIPP",;
									"PP",;
									"C",;
									"Prefixo utilizado na criação da NCC do Price Protection",;
									"Prefixo utilizado na criação da NCC do Price Protection",;
									"Prefixo utilizado na criação da NCC do Price Protection",;
									.F. )
...
If (!Empty(cCodCli)) .And. (!Empty(cLoja)) .And. (!Empty(cCodPP))
	...
	If (cArqTmp)->(!Eof())
		...
		If Empty(cRet)
			...
			Else                                                                                                                  
				cRet := "Erro na criação da NCC - Cod.PP: "+cCodPP+"  |Titulo: "+cNumTit+" |Prefixo: "+cPrefixo+" |Parcela:  |Tipo: NCC "+CRLF
				cRet += " Não foi possivel efetuar amarração PP x NCC "+CRLF+CRLF
			EndIf
	   	Else
			Aviso("ERRNCC-3","Erro ao tentar criar NCC para o cliente "+cCodCli+"/"+cLoja+;
						" no documento de Price Protection de n.º: "+cCodPP+CRLF+CRLF+cRet,{"Ok"},3)
			RollBackSx8()
		EndIf
		
	Else
		Aviso("ERRNCC-2","Nenhum item encontrado para o cliente "+cCodCli+"/"+cLoja+" no documento de Price Protection de n.º: "+cCodPP,{"Ok"},2)
	EndIf
	
	
	(cArqTmp)->(DbCloseArea())
Else
	Aviso("ERRNCC-1","Erro ao gerar NCC. Itens obrigatórios não preenchidos: "+CRLF+"Cliente: "+cCodCli+CRLF+" Loja: "+cLoja+CRLF+" Cod.PP: "+cCodPP,{"Ok"},3)
EndIf
...
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³Grava os dados do controle de repasse (NCC X PP)			  º±±
±±º          ³					  						  				  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
...
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Seleciona os clientes listados no documento do PP			  º±±
±±º          ³             												  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³			                                                  º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
