...
#DEFINE LITERAL_ALTERACAO          IF( nIdioma == INGLES, "REVISION Number: ", STR0002 ) //"ALTERACAO Numero: "
#DEFINE LITERAL_DATA               IF( nIdioma == INGLES, "Date: "             , STR0003 ) //"Data: "
#DEFINE LITERAL_PAGINA             IF( nIdioma == INGLES, "Page: "             , STR0004 ) //"Pagina: "
#DEFINE LITERAL_FORNECEDOR         IF( nIdioma == INGLES, "SUPPLIER........: " , STR0005 ) //"FORNECEDOR......: "
#DEFINE LITERAL_ENDERECO           IF( nIdioma == INGLES, "ADDRESS.........: " , STR0006 ) //"ENDEREÇO........: "
...
#DEFINE LITERAL_COMISSAO           IF( nIdioma == INGLES, "COMMISSION......: " , STR0009 ) //"COMISSÃO........: "
...
#DEFINE LITERAL_COD_BARRAS         IF( nIdioma == INGLES, "UPC"                , "Cod. Barras" ) //AR 25/08/10
#DEFINE LITERAL_DESCRICAO          IF( nIdioma == INGLES, "Description"        , STR0017 ) //"Descrição"
#DEFINE LITERAL_FABRICANTE         IF( nIdioma == INGLES, "Manufacturer"       , STR0018 ) //"Fabricante"
#DEFINE LITERAL_PRECO_UNITARIO1    IF( nIdioma == INGLES, "Unit"               , STR0019 ) //"Preco"
#DEFINE LITERAL_PRECO_UNITARIO2    IF( nIdioma == INGLES, "Price"              , STR0020 ) //"Unitario"
...
#DEFINE LITERAL_OBSERVACOES        IF( nIdioma == INGLES, "REMARKS"            , STR0024 ) //"OBSERVACOES"
...
#xtranslate :COURIER_10_NEGRITO            => \[4\]
...
#xtranslate :COURIER_NEW_08_NEGRITO        => \[10\]
#xtranslate :TIMES_NEW_ROMAN_08            => \[11\] //AR 27/07/10
#xtranslate :COURIER_08                    => \[12\] //AR 27/07/10
...
cPictQtde:='@E 999,999,999.99'; cPict1Total:='@E 999,999,999,999.99'
...
ELSE
   ...
   AVPRINT oPrn NAME STR0039 //"EmissÒo do Pedido"
      ...
      oFont11:= oSend(TFont(),"New","Times New Roman",0,08,,.F.,,,,,,,,,,,oPrn )
      oFont12:= oSend(TFont(),"New","Courier New"    ,0,08,,.F.,,,,,,,,,,,oPrn )
...
*--------------------------*
Local nMarcados
Local nPosKit //AR 18/08/10
Private aItemJaUsado := {} //AR 18/08/10
Private aItensSemKit := {} //AR 20/08/10
Private aItensSW3    := {} //AR 20/08/10
...
For nMarcados:=1 To Len(aMarcados)
   ...
   While SW3->(!Eof()) .AND.;
      ...
      //PO150Item() - AR 18/08/10 - Flegado em 18/08/10
      
      //AR 18/08/10
      nPosKit := aScan(aItemJaUsado,SW3->(RecNo()))
      If nPosKit > 0
         SW3->(DbSkip())
         Loop
      Else
...
c2EndSYT := c2EndSYT + IF( !EMPTY(SYT->YT_CEP   ), "CEP: " + TRANS(SYT->YT_CEP,"@R 99999-999")+' - ', "" ) //AR 23/07/10
...
//AR 18/08/10 - Inclusao de Logotipo
cEmp := "NCGAMES.BMP"
//oSend(oPrn, "SayBitmap", Linha+30, 50, cEmp , 550, 240 )
oSend(oPrn, "SayBitmap", Linha+60, 50, cEmp , 580, 170 )
...
// oSend( oPrn, "Say", Linha    , 1150, ALLTRIM(SM0->M0_ENDCOB), aFontes:TIMES_NEW_ROMAN_12,,,, 2 )
   //oSend( oPrn, "Say", Linha    , 1150, ALLTRIM(SM0->M0_ENDCOB) + " - " + ALLTRIM(SM0->M0_BAIRCOB), aFontes:TIMES_NEW_ROMAN_12,,,, 2 )
//ELSE
...
// oSend( oPrn, "Say", Linha    , 1150, ALLTRIM(SYT->YT_ENDE)+ " " + cNr, aFontes:TIMES_NEW_ROMAN_12,,,, 2 )
   oSend( oPrn, "Say", Linha    , 1150, ALLTRIM(SYT->YT_ENDE)+ " " + cNr + " - " + ALLTRIM(SYT->YT_BAIRRO), aFontes:TIMES_NEW_ROMAN_12,,,, 2 )
//ENDIF

oSend( oPrn, "Say", Linha:= Linha+50, 1150, ALLTRIM(c2EndSM0), aFontes:TIMES_NEW_ROMAN_12,,,, 2 )
Linha += 50 //AR 27/07/10
//If GetMv("MV_ID_EMPR") == 'S'
   //oSend( oPrn, "Say",  Linha, 1150, AVSX3("YT_CGC",5)+": "  + Trans(SM0->M0_CGC,"@R 99.999.999/9999-99") , aFontes:TIMES_NEW_ROMAN_12,,,, 2 ) // "C.N.P.J. "
//Else
   oSend( oPrn, "Say",  Linha, 1150, AVSX3("YT_CGC",5)+": "  + Trans(cCGC,"@R 99.999.999/9999-99") , aFontes:TIMES_NEW_ROMAN_12,,,, 2 ) // "C.N.P.J. "
//EndIf
Linha += 50 //AR 27/07/10

//AR 23/07/10 - FLEGADO OS TELEFONES DOS COMPRADORES
/*
...
oSend( oPrn, "Say",  Linha, 630, TRANS(SA2->A2_CEP,"@R 99999-999"), aFontes:TIMES_NEW_ROMAN_12 )
Linha := Linha+50
...
oSend( oPrn, "Say",  Linha, 630, ALLTRIM(SYT->YT_ENDE) + " " + cNr + " - " + ALLTRIM(SYT->YT_BAIRRO), aFontes:TIMES_NEW_ROMAN_12 )
...
//TRP-12/08/08
If lNewProforma .and. GetMv("MV_AVG0186") // NCF - 05/01/2010 - Criação do Parâmetro MV_AVG0186 que define se as informações da proforma
...
oSend( oPrn, "Say",  Linha,  140, LITERAL_QUANTIDADE          , aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
oSend( oPrn, "Say",  Linha,  430, LITERAL_COD_BARRAS          , aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
//oSend( oPrn, "Say",  Linha,  480, LITERAL_DESCRICAO      , aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
oSend( oPrn, "Say",  Linha,  690, LITERAL_DESCRICAO           , aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
//oSend( oPrn, "Say",  Linha, 1500, LITERAL_FABRICANTE     , aFontes:TIMES_NEW_ROMAN_10_NEGRITO,,,,1 )
oSend( oPrn, "Say",  Linha, 1340, LITERAL_FABRICANTE          , aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
//oSend( oPrn, "Say",  Linha, 1530, LITERAL_PRECO_UNITARIO1, aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
oSend( oPrn, "Say",  Linha, 1650, LITERAL_PRECO_UNITARIO1     , aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
//oSend( oPrn, "Say",  Linha, 1840, LITERAL_TOTAL_MOEDA    , aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
oSend( oPrn, "Say",  Linha, 1870, AllTrim(LITERAL_TOTAL_MOEDA), aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
...
//oSend( oPrn, "Say",  Linha, 1530, LITERAL_PRECO_UNITARIO2, aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
oSend( oPrn, "Say",  Linha, 1650, LITERAL_PRECO_UNITARIO2, aFontes:TIMES_NEW_ROMAN_10_NEGRITO )
...
*-------------------------*
Local i
Local lKit    := .F. //AR 18/08/10
Local nQtde   := nPreco := 0 //AR 18/08/10
Local nRecSW3 := SW3->(Recno()) //AR 18/08/10
Local nPosSemKit, nI //AR 18/08/10
Local cCodBarra //AR 25/08/10
...
//AR - 19/08/2010
nPosSemKit := aScan(aItensSemKit, SW3->(Recno()))
If nPosSemKit > 0
   lKit := .F.
Else
   lKit := MontaArray()
...
If lKit
   SB1->( DbSetOrder(1), DbSeek(xFilial("SB1") + AvKey(aItensSW3[1][1],"B1_COD") ) ) //Código do Kit
EndIf

cCodBarra := SB1->B1_CODBAR
//AR - FIM

cDescrItem := MSMM(IF( nIdioma==INGLES, SB1->B1_DESC_I, SB1->B1_DESC_P ),nTam)
...
If lKit //AR 20/08/10
   nQtde  := aItensSW3[1][3]
   nPreco := aItensSW3[1][4]
   For nI:=2 To Len(aItensSW3)
      nPreco += aItensSW3[nI][4]
   Next
      
   oSend( oPrn, "Say",  Linha, 320, TRANS(nQtde,E_TrocaVP(nIdioma,cPictQtde)), aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
   oSend( oPrn, "Say",  Linha, 340, BUSCA_UM(SW3->W3_COD_I+SW3->W3_FABR +SW3->W3_FORN,SW3->W3_CC+SW3->W3_SI_NUM), aFontes:TIMES_NEW_ROMAN_08_NEGRITO )
   oSend( oPrn, "Say",  Linha, 440, PADR(cCodBarra,15), aFontes:TIMES_NEW_ROMAN_08_NEGRITO ) //AR 25/08/10
   oSend( oPrn, "Say",  Linha, 690, MEMOLINE( cDescrItem,nTam ,1 ), aFontes:COURIER_NEW_08_NEGRITO )
   oSend( oPrn, "Say",  Linha,1340, PADR(LEFT(SA2->A2_NREDUZ,15),15), aFontes:COURIER_NEW_08_NEGRITO )
   oSend( oPrn, "Say",  Linha,1840, TRANS(nPreco,E_TrocaVP(nIdioma,'@E 999,999,999.99')), aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
   oSend( oPrn, "Say",  Linha,2100, TRANS(nQtde * nPreco,E_TrocaVP(nIdioma,cPict1Total )), aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
   oSend( oPrn, "Say",  Linha,2130, DATA_MES(SW3->W3_DT_EMB), aFontes:TIMES_NEW_ROMAN_08_NEGRITO )
   
   nTotal := DI500TRANS(nTotal + nQtde * nPreco,2)
Else
   //oSend( oPrn, "Say",  Linha, 370, TRANS(SW3->W3_QTDE,E_TrocaVP(nIdioma,cPictQtde)), aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
   oSend( oPrn, "Say",  Linha, 320, TRANS(SW3->W3_QTDE,E_TrocaVP(nIdioma,cPictQtde)), aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
   /*
   ...
   */
   ...
   //If MLCOUNT(cDescrItem,nTam) == 1 .OR. lPoint1P
      //oSend( oPrn, "Say",  Linha, 480, MEMOLINE( cDescrItem,nTam ,1 ), aFontes:COURIER_NEW_08_NEGRITO )
   //Else
   // oSend( oPrn, "Say",  Linha, 480, AV_Justifica(MEMOLINE( cDescrItem,nTam ,1 )), aFontes:COURIER_NEW_08_NEGRITO )
      oSend( oPrn, "Say",  Linha, 690, MEMOLINE( cDescrItem,nTam ,1 ), aFontes:COURIER_NEW_08_NEGRITO )
   //EndIf
   //oSend( oPrn, "Say",  Linha,1500, LEFT(SA2->A2_NREDUZ,10), aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
   oSend( oPrn, "Say",  Linha,1340, PADR(LEFT(SA2->A2_NREDUZ,15),15), aFontes:COURIER_NEW_08_NEGRITO )
   //oSend( oPrn, "Say",  Linha,1740, TRANS(SW3->W3_PRECO,E_TrocaVP(nIdioma,'@E 999,999,999.99')), aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
   oSend( oPrn, "Say",  Linha,1840, TRANS(SW3->W3_PRECO,E_TrocaVP(nIdioma,'@E 999,999,999.99')), aFontes:TIMES_NEW_ROMAN_08_NEGRITO,,,,1 )
...
//SVG 18/11/08            
//AR 18/08/10 - nTotal := DI500TRANS(nTotal + SW3->W3_QTDE*SW3->W3_PRECO,2)
Linha  := Linha + 50
...
FOR i:=2 TO n1 + 1   // Soma um para bater o ultimo.
    ...
    IF !EMPTY( MEMOLINE( cDescrItem,nTam, i ) )
       ...
       //   oSend( oPrn, "Say",  Linha, 480,MEMOLINE( cDescrItem,nTam,i ), aFontes:COURIER_NEW_08_NEGRITO )
       //ELSE
//        oSend( oPrn, "Say",  Linha, 480,AV_Justifica( MEMOLINE( cDescrItem,nTam,i ) ), aFontes:COURIER_NEW_08_NEGRITO )
          oSend( oPrn, "Say",  Linha, 690,MEMOLINE( cDescrItem,nTam,i  ), aFontes:COURIER_NEW_08_NEGRITO )
       //ENDIF
       lPulaLinha := .T.
    ENDIF

//AR - 24/08/10 - BLOCO DE CÓDIGO COMENTADO
/*
    IF EMPTY( MEMOLINE( cDescrItem,nTam, i ) )
       IF nNumero == 1         
...
*----------------------------*
Local i
Local nLinhaObs := 800 //AR 27/07/10
...
//FOR i:=1 TO MIN(MLCOUNT( cRemarks, 40 ),7) 
FOR i:=1 TO MIN(MLCOUNT( cRemarks, 60 ),12) //AR 27/07/10 
   
   //IF !EMPTY(MEMOLINE( cRemarks,40, i ))
   IF !EMPTY(MEMOLINE( cRemarks,60, i ))
      //oSend( oPrn, "Say",  Linha, 065 , MEMOLINE( cRemarks,40, i ),  aFontes:TIMES_NEW_ROMAN_08_NEGRITO )
      oSend( oPrn, "Say",  Linha, 065 , MEMOLINE( cRemarks,60, i ),  aFontes:COURIER_08 ) //AR 27/07/10
   ENDIF 

Linha     := Linha+50
nLinhaObs -= 50 //AR 27/07/10

NEXT

Linha += nLinhaObs //AR 27/07/10
...
oSend( oPrn, "Say",  Linha, 1250 , STR0044                                   , aFontes:COURIER_10_NEGRITO ) //"TOTAL "
nTLinha := Linha 
...
oSend( oPrn, "Say",  Linha, 1250 , LITERAL_INLAND_CHARGES , aFontes:COURIER_10_NEGRITO )
...
oSend( oPrn, "Say",  Linha, 1250 , LITERAL_PACKING_CHARGES , aFontes:COURIER_10_NEGRITO )
...
oSend( oPrn, "Say",  Linha, 1250 , LITERAL_INTL_FREIGHT , aFontes:COURIER_10_NEGRITO )
...
pTipo := 6
PO150BateTraco()
oSend( oPrn, "Say",  Linha, 1250 , LITERAL_DISCOUNT , aFontes:COURIER_10_NEGRITO )
...
pTipo := 6
PO150BateTraco()
oSend( oPrn, "Say",  Linha, 1250 , LITERAL_OTHER_EXPEN , aFontes:COURIER_10_NEGRITO )
...
oSend( oPrn, "Say",  Linha, 1250 , STR0044 + ALLTRIM( SW2->W2_INCOTER )         , aFontes:COURIER_10_NEGRITO ) //"TOTAL "
oSend( oPrn, "Say",  Linha, 1675 , SW2->W2_MOEDA,aFontes:COURIER_10_NEGRITO )
...
DO CASE
   ...
   CASE pTipo == 2  .OR.  pTipo == 4  .OR.  pTipo == 5
        ...
        oPrn:Box( Linha, 410, (Linha+xLinha), 411) //AR 25/08/2010
        oPrn:Box( Linha, 670, (Linha+xLinha), 671) //AR 25/08/2010
        oPrn:Box( Linha,1320, (Linha+xLinha),1321) //AR 25/08/2010
        //oPrn:Box( Linha,1510, (Linha+xLinha),1511)
        oPrn:Box( Linha,1630, (Linha+xLinha),1631) //AR 25/08/2010
        //oPrn:Box( Linha,1750, (Linha+xLinha),1751)
        oPrn:Box( Linha,1850, (Linha+xLinha),1851) //AR 25/08/2010
...
*----------------------------------------*
Local nQtde, nPos, nPosJaUsado, nPosSemKit, cKit, cCC, cSI, nI
Local lKit      := .F.
Local aItensKit := {}

aItensSW3 := {}

If SG1->(DbSetOrder(2), DbSeek(xFilial("SG1") + AvKey(SW3->W3_COD_I,"G1_COMP"))) //Se o item é componente de alguma estrutura
   //Ver se o item está repetido no SG1
   SG1->(DbSkip())
   If SG1->G1_COMP <> AvKey(SW3->W3_COD_I,"G1_COMP")
      SG1->(DbSkip(-1))
      aAdd(aItensSW3, { SG1->G1_COD, SW3->W3_COD_I, (SW3->W3_QTDE / SG1->G1_QUANT), (SW3->W3_PRECO * SG1->G1_QUANT), SW3->(Recno()) })
   Else
      MsgInfo("Produto: " + AllTrim(SW3->W3_COD_I) + " cadastrado em mais de um Kit! Corrija o cadastro de Estrutura " +;
              "de Produtos (Módulo: Estoque/Custos).", "P.O.: " + SW2->W2_PO_NUM)
      aAdd(aItensSemKit, SW3->(Recno()))
      Return lKit
   EndIf

   nQtde := SW3->W3_QTDE / SG1->G1_QUANT
   cKit  := SG1->G1_COD
   cCC   := SW3->W3_CC
   cSI   := SW3->W3_SI_NUM
   ...
   While SW3->(!Eof()) .And.;
         SW3->W3_FILIAL == xFilial("SW3") .And.;
         SW3->W3_PO_NUM == SW2->W2_PO_NUM .And.;
         SW3->W3_CC     == cCC .And.;
         SW3->W3_SI_NUM == cSI
      ...
      If SW3->W3_SEQ # 0
         ...
         Loop
      Else
         nPos        := aScan( aItensSW3,    {|x| x[2] == SW3->W3_COD_I} )
         nPosJaUsado := aScan( aItemJaUsado, SW3->(Recno()) )
         nPosSemKit  := aScan( aItensSemKit, SW3->(Recno()) )
         If nPos > 0 .Or. nPosJaUsado > 0 .Or. nPosSemKit > 0
            SW3->(DbSkip())
            Loop
         ElseIf SG1->(DbSetOrder(2), DbSeek(xFilial("SG1") + AvKey(SW3->W3_COD_I,"G1_COMP")))
            SG1->(DbSkip())
            If SG1->G1_COMP <> AvKey(SW3->W3_COD_I,"G1_COMP")
               SG1->(DbSkip(-1))
               If cKit == SG1->G1_COD .And. nQtde == (SW3->W3_QTDE / SG1->G1_QUANT)
                  aAdd(aItensSW3, { SG1->G1_COD, SW3->W3_COD_I, nQtde, (SW3->W3_PRECO * SG1->G1_QUANT), SW3->(Recno()) })
               EndIf
            Else
               MsgInfo("Produto: " + AllTrim(SW3->W3_COD_I) + " cadastrado em mais de um Kit! Corrija o cadastro de Estrutura " +;
                       "de Produtos (Módulo: Estoque/Custos).", "P.O.: " + SW2->W2_PO_NUM)
   ...
   If Len(aItensSW3) <> Len(aItensKit) .Or. Len(aItensSW3) == 1
      For nI:=1 To Len(aItensSW3)
         aAdd(aItensSemKit, aItensSW3[nI][5]) //Recno do SW3
      Next
      Return lKit
   Else
      aSort( aItensSW3,,, { |x,y| x[2] < y[2] } )
      aSort( aItensKit,,, { |x,y| x[2] < y[2] } )
      For nI:=1 To Len(aItensSW3)
         If aItensSW3[nI][2] <> aItensKit[nI][2]
            MsgInfo("Produtos divergentes para a formação do Kit: " + AllTrim(cKit) + ". Verifique os produtos.",;
                    "P.O.: " + SW2->W2_PO_NUM)
            Return lKit
         EndIf
      Next
      ...
      For nI:=1 To Len(aItensSW3)
         aAdd(aItemJaUsado, aItensSW3[nI][5]) //Recno do SW3
      Next
