			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					cCodProd,;
					IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
					AllTrim(SB1->B1_XDESC)+iif(Val(SB1->B1_CODBAR)!=0," - Cod.Barra: "+AllTrim(SB1->B1_CODBAR),""),;//					SB1->B1_DESC,; //alterado nc games SB1->B1_DESC,;
					SB1->B1_POSIPI,;
					SB1->B1_EX_NCM,;
					(cAliasSD1)->D1_CF,;
					SB1->B1_UM,;
...
IF ExistBlock("PE01NFESEFAZ")                   
	...
	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque, aRefECF}
...
cString += '<vUnCom>'+ConvType(aProd[10]/aProd[12],21,8)+'</vUnCom>'	
Else                                       
