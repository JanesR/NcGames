...
ฑฑบPrograma  ณDBESPCO2 บ Autor ณ Alberto Kibino    บ Data ณ  29/10/12     บฑฑ
...
If lAuto	
	alData := {DtoS(Firstday(ddatabase)),DtoS(Lastday(ddatabase))}
	cQano := StrZero(Month(dDataBase),2) + MV_PAR02
	If Month(dDataBase) == 12 
		cDtLimite := AllTrim(MV_PAR02) + "01" + StrZero(GETMV("DBM_DTFCES"),2)
	Else
		cDtLimite := AllTrim(MV_PAR02) + StrZero(Month(dDataBase)+1,2) + StrZero(GETMV("DBM_DTFCES"),2) 
	EndIF
Else
	alData := {DtoS(Firstday(StoD(MV_PAR02+ StrZero(MV_PAR01,2) + "01"))),DtoS(Lastday(StoD(MV_PAR02 + StrZero(MV_PAR01,2)+"01")))}
	cQano := StrZero(MV_PAR01,2) + MV_PAR02  
	If MV_PAR01 == 12 
		cDtLimite := StrZero(Val(MV_PAR02)+1,4) + "01" + StrZero(GETMV("DBM_DTFCES"),2)
	Else
		cDtLimite := AllTrim(MV_PAR02) + StrZero(Month(dDataBase)+1,2) + StrZero(GETMV("DBM_DTFCES"),2) 
	EndIf
EndIf
...
_cQry+=" AND	ACA_GRPREP BETWEEN '" + cGCanal1 + "' AND '" + cGCanal2 + "'"  
...
While !(clAlias)->(EOF())     
	...
	If SZ1->(Dbseek(xFilial("SZ1") + (clAlias)->F2_DOC + (clAlias)->F2_SERIE + (clAlias)->A1_COD + (clAlias)->A1_LOJA))
		...
		Else
			If Empty(SZ1->Z1_ANOMESC) 
				If SZ1->Z1_DTENTRE > Stod(cDtLimite)
					cCalcPr := "N"
					cObsCalc := "Ultrapassado data limite de entrega"
				Else
					If RecLock("SZ1",.F.)
						SZ1->Z1_ANOMESC := cQano
						SZ1->(MsUnLock())
						cCalcPr := "S"
						cObsCalc := "" 
					Else
				...
				EndIf                         
			Else
				If AllTrim(SZ1->Z1_ANOMESC) == AllTrim(cQano)
					cCalcPr := "S"
					cObsCalc := ""
				Else
					cCalcPr := "N" 
					cObsCalc := "CALCULADO NO MสS "+ substr(SZ1->Z1_ANOMESC,1,2) +"." 
				EndIf
			EndIf
		EndIf
	Else
	...
	If DGXLS->(RECLOCK("DGXLS",.T.))
		...
		Else
			If MV_PAR01 == 12 
				If Dtos(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)+1))  + "01" + StrZero(GETMV("DBM_DTFCES"),2))
					DGXLS->MES := "12"
				Else
					DGXLS->MES := "XX"
				EndIf
			Else 
				If Dtos(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + StrZero(MV_PAR01+1,2) + StrZero(GETMV("DBM_DTFCES"),2))
					DGXLS->MES := StrZero(MV_PAR01,2)
				Else
					DGXLS->MES := "XX"
				EndIf
			EndIf		
		...
		DGXLS->VALFAT		:= (clAlias)->(D2_VALFRE + D2_SEGURO + D2_DESPESA +  D2_ICMSRET + D2_VALIPI + D2_TOTAL)
...
EndIf
cSqlSZ1+= " AND (Z1_ANOMESC = '' Or Z1_ANOMESC = '"+ AllTrim(cQano) + "')"
cSqlSZ1+=" AND	ACA_GRPREP BETWEEN '" + cGCanal1 + "' AND '" + cGCanal2 + "'"  
		...
		Else
			...
			Else
				...
				Else
					...
					cCalcPr := "N" 
					cObsCalc := "CALCULADO NO MสS"+ substr(SZ1->Z1_ANOMESC,1,1) +"บ QUARTER" 
				EndIf
			EndIf
		EndIf
	Else
		...
		If SA3->(dbSeek(xFilial("SA3") + (cAliSZ1)->F2_VEND1))  
			If ACA->(dbSeek(xFilial("ACA") + SA3->A3_GRPREP))		 
				...
				//	DGXLS->CONDPG		:= (cAliSZ1)->E4_DESCRI
					...
					DGXLS->VALFAT		:= (cAliSZ1)->(D2_VALFRE + D2_SEGURO + D2_DESPESA + D2_ICMSRET + D2_VALIPI + D2_TOTAL)
...
cSqlDev += " AND A3_GRPREP BETWEEN '" + cGCanal1 + "' AND '" + cGCanal2 + "'"
...
PutSx1("DBM_ESPCO2","01","Mes Base"	   		,"Quarter Base"		,"Quarter Base"  	,"mv_ch1","N",02,0,1,"G","(mv_par01 < 13 .And. mv_par01 > 0 )",""   ,"","","mv_par01","","","","","","","","","","","","","","","","",{},{},{})
PutSx1("DBM_ESPCO2","02","Ano Base"	   		,"Ano Base"			,"Ano Base"  		,"mv_ch2","C",04,0,1,"G","",""   ,"","","mv_par02","","","","","","","","","","","","","","","","",{},{},{})
PutSx1("DBM_ESPCO2","03","Canal de"			,"Canal de"	   		,"Canal de"	    	,"mv_ch3","C",06,0,1,"G","","ACA","","","mv_par03","","","","","","","","","","","","","","","","",{},{},{})
PutSx1("DBM_ESPCO2","04","Canal ate"		,"Canal ate"	   	,"Canal ate"	    ,"mv_ch4","C",06,0,1,"G","","ACA","","","mv_par04","","","","","","","","","","","","","","","","",{},{},{})
