...
±±ºPrograma  ³AtuSZC    ºAutor  ³Microsiga           º Data ³  05/10/12   º±±
...
For nn:=1 to len(aZAAtiv)
	...
	cQry += CRLF+" QDEV_DIA,
	cQry += CRLF+" DEVVALBRUTO_DIA DEVBR_DIA,
	cQry += CRLF+" DEVVLSIMP_DIA DVSIMPDIA,
	...
	cQry += CRLF+" 	(VLSIMPT - DEVVLSIMPT)-(CMVFTTOT-CMVDVTOT) LBRUT_ACM,
	cQry += CRLF+" 	CASE WHEN CMVFTTOT-CMVDVTOT = 0 THEN 0 ELSE ROUND((VLSIMPT - DEVVLSIMPT)/(CMVFTTOT-CMVDVTOT),2) END MARKUP_TOT,
	cQry += CRLF+" 	CASE WHEN DEVVLSIMPT = 0
	cQry += CRLF+" 		THEN ROUND(((VLSIMPT)-(CMVFTTOT))-((VLSIMPT)*("+alltrim(str(nfator))+"/100)),2)
	cQry += CRLF+" 		ELSE ROUND(((VLSIMPT - DEVVLSIMPT)-(CMVFTTOT-CMVDVTOT))-((VLSIMPT - DEVVLSIMPT)*("+alltrim(str(nfator))+"/100)),2) END LLIQ_ACM
	cQry += CRLF+" 		FROM
	cQry += CRLF+" 		(SELECT ZB_COD CODCAMPFAT,
	cQry += CRLF+" 			ZB_CODPRO PRODUTOFAT,
	cQry += CRLF+" 			CASE WHEN VALMERC IS NULL THEN 0 ELSE VALMERC END VALMERC,
	...
	SZC->ZC_VDVDIA	:= cArqSZC->DEVBR_DIA
	SZC->ZC_DVSIMP	:= cArqSZC->DVSIMPDIA
