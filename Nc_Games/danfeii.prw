...
#DEFINE MAXITEMP2  030//049 -- ESPECIFICO NC GAMES                                                // Máximo de produtos para a pagina 2 em diante
...
Private	lGATI		:=IsIncallStack("PR118Exec")
Private	lEcom		:=IsIncallStack("U_NCECOM11") .Or. IsIncallStack("U_NCGWM002") .Or. IsIncallStack("U_NcgPr125") .Or. IsIncallStack("U_PrTRFDanf")
...
If IIf(lGATI,.T.,IIF(lEcom,.T.,Pergunte("NFSIGW",.T.)))
...
//Inicio - Alteração NC Games
Local cRazaAux		:= ""
Local cCnpjAux		:= ""
//Fim - Alteração NC Games
Default cDtHrRecCab := ""
...
Do Case
	Case Type("oDestino:_CNPJ")=="O"
		cCnpjAux := TransForm(oDestino:_CNPJ:TEXT,"@r 99.999.999/9999-99")
	Case Type("oDestino:_CPF")=="O"
		cCnpjAux := TransForm(oDestino:_CPF:TEXT,"@r 999.999.999-99")
	OtherWise
		cCnpjAux := Space(14)
EndCase
...
If !Empty(cCnpjAux)
	cCnpjAux := STRTRAN(cCnpjAux,".","")
	cCnpjAux := STRTRAN(cCnpjAux,"/","")
	cCnpjAux := STRTRAN(cCnpjAux,"-","")
	
	DbSelectArea("SA1")
	DbSetOrder(3)
	If SA1->(MsSeek(xFilial("SA1")+ cCnpjAux ))
		cRazaAux := Alltrim(SA1->A1_NREDUZ)
	EndIf
	
	If !Empty(cRazaAux)
		Aadd(aMensagem, "Nome Fantasia: "+cRazaAux)
	EndIf
EndIf	
