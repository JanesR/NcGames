...
ฑฑบPrograma  ณDBKEYCOM บ Autor ณ Alberto Kibino    บ Data ณ  29/10/12     บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDescricao ณ RELATORIO PARA GERAR DBF PARA RELATึRIO de COMISSOES       บฑฑ
ฑฑบ          ณ Key Account			                                      บฑฑ
...
If MV_PAR01 == 1
 	...
 	cQano := "1" + MV_PAR02 
 	cDtLimite := AllTrim(MV_PAR02) + "04" + StrZero(GETMV("DBM_DTFCKA"),2)
ElseIf MV_PAR01 == 2
 	alData := {DtoS(Firstday(StoD(MV_PAR02+"0401"))),DtoS(Lastday(StoD(MV_PAR02+"0601")))}
	cQano := "2" + MV_PAR02
	cDtLimite := AllTrim(MV_PAR02) + "07" + StrZero(GETMV("DBM_DTFCKA"),2)
ElseIf MV_PAR01 == 3
 	alData := {DtoS(Firstday(StoD(MV_PAR02+"0701"))),DtoS(Lastday(StoD(MV_PAR02+"0901")))}
 	cQano := "3" + MV_PAR02 
 	cDtLimite := AllTrim(MV_PAR02) + "10" + StrZero(GETMV("DBM_DTFCKA"),2)
Else
 	alData := {DtoS(Firstday(StoD(MV_PAR02+"1001"))),DtoS(Lastday(StoD(MV_PAR02+"1201")))}
 	cQano := "4" + MV_PAR02 
 	cDtLimite := AllTrim(Str(val(MV_PAR02)+1)) + "01" + StrZero(GETMV("DBM_DTFCKA"),2)
EndIF
...
MyNewSX6( "DBM_KEYACC",  "000001", "C", "Grupo Key Account","Grupo Key Account","Grupo Key Account", .F. )
...
cGrpCli := AllTrim(GetMv("DBM_KEYACC"))
...
While !(clAlias)->(EOF())     
	...
	If DGXLS->(RECLOCK("DGXLS",.T.))
		...
		Else
			//StrZero(GETMV("DBM_DTFCKA"),2)  
			If MV_PAR01 == 1 
				If Dtos(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "02" + StrZero(GETMV("DBM_DTFCKA"),2))
					If Month(StoD((clAlias)->F2_EMISSAO)) == 2
						DGXLS->MES := "02"
					Else
						DGXLS->MES := "01"
					EndIf	
				ElseIf DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "03" + StrZero(GETMV("DBM_DTFCKA"),2))
					If Month(StoD((clAlias)->F2_EMISSAO)) == 3
						DGXLS->MES := "03"
					Else
						DGXLS->MES := "02"
					EndIf
				ElseIf DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "04" + StrZero(GETMV("DBM_DTFCKA"),2))
			...
			If MV_PAR01 == 2 
				If DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "05" + StrZero(GETMV("DBM_DTFCKA"),2))
					If Month(StoD((clAlias)->F2_EMISSAO)) == 5
						DGXLS->MES := "05"
					Else
						DGXLS->MES := "04"
					EndIf	
				ElseIf DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "06" + StrZero(GETMV("DBM_DTFCKA"),2))
					If Month(StoD((clAlias)->F2_EMISSAO)) == 6
						DGXLS->MES := "06"
					Else
						DGXLS->MES := "05"
					EndIf
				ElseIf DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "07" + StrZero(GETMV("DBM_DTFCKA"),2))
			...
			If MV_PAR01 == 3 
				If DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "08" + StrZero(GETMV("DBM_DTFCKA"),2))  
					If Month(StoD((clAlias)->F2_EMISSAO)) == 8
						DGXLS->MES := "08"
					Else
						DGXLS->MES := "07"
					EndIf	
				ElseIf DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "09" + StrZero(GETMV("DBM_DTFCKA"),2))
					If Month(StoD((clAlias)->F2_EMISSAO)) == 9
						DGXLS->MES := "09"
					Else
						DGXLS->MES := "08"
					EndIf
				ElseIf DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "10" + StrZero(GETMV("DBM_DTFCKA"),2)) 
			...
			If MV_PAR01 == 4 
				If DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "11" + StrZero(GETMV("DBM_DTFCKA"),2))
					If Month(StoD((clAlias)->F2_EMISSAO)) == 11
						DGXLS->MES := "11"
					Else
						DGXLS->MES := "10"
					EndIf	
				ElseIf DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)))  + "12" + StrZero(GETMV("DBM_DTFCKA"),2))
					If Month(StoD((clAlias)->F2_EMISSAO)) == 12
						DGXLS->MES := "12"
					Else
						DGXLS->MES := "11"
					EndIf
				ElseIf DtoS(dtEntreg) <= (AllTrim(Str(Year(cl_PAR12)+1))  + "01" + StrZero(GETMV("DBM_DTFCKA"),2))
					DGXLS->MES := "12"
				EndIf
			EndIf
...
If SubStr(cQano,1,1) = "1"
	cSqlSZ1+= " AND F2_EMISSAO BETWEEN '" + StrZero(Year(cl_PAR12)-1,4) + "10" + "01' AND '" + DTOS(cl_PAR12-1) + "'"
Else
	cSqlSZ1+= " AND F2_EMISSAO BETWEEN '" + StrZero(Year(cl_PAR12),4) + StrZero(Month(cl_PAR12)-3,2) + "01' AND '" + DTOS(cl_PAR12-1) + "'"
EndIf
		...
		If SA3->(dbSeek(xFilial("SA3") + (cAliSZ1)->F2_VEND1))  
			If ACA->(dbSeek(xFilial("ACA") + SA3->A3_GRPREP))		 
					If DGXLS->(RECLOCK("DGXLS",.T.))
						...
						If MV_PAR01 == 1 
							If (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "02" + StrZero(GETMV("DBM_DTFCKA"),2))
								If Month(StoD((cAliSZ1)->F2_EMISSAO)) == 2
									DGXLS->MES := "02"
								Else
									DGXLS->MES := "01"
								EndIf	
							ElseIf (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "03" + StrZero(GETMV("DBM_DTFCKA"),2))
								If Month(StoD((cAliSZ1)->F2_EMISSAO)) == 3
									DGXLS->MES := "03"
								Else
									DGXLS->MES := "02"
								EndIf
							ElseIf (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "04" + StrZero(GETMV("DBM_DTFCKA"),2))
						...
						If MV_PAR01 == 2 
							If (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "05" + StrZero(GETMV("DBM_DTFCKA"),2))
								If Month(StoD((cAliSZ1)->F2_EMISSAO)) == 5
									DGXLS->MES := "05"
								Else
									DGXLS->MES := "04"
								EndIf	
							ElseIf (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "06" + StrZero(GETMV("DBM_DTFCKA"),2))
								If Month(StoD((cAliSZ1)->F2_EMISSAO)) == 6
									DGXLS->MES := "06"
								Else
									DGXLS->MES := "05"
								EndIf
							ElseIf (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "07" + StrZero(GETMV("DBM_DTFCKA"),2))
						...
						If MV_PAR01 == 3 
							If (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "08" + StrZero(GETMV("DBM_DTFCKA"),2))  
								If Month(StoD((cAliSZ1)->F2_EMISSAO)) == 8
									DGXLS->MES := "08"
								Else
									DGXLS->MES := "07"
								EndIf	
							ElseIf (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "09" + StrZero(GETMV("DBM_DTFCKA"),2))
								If Month(StoD((cAliSZ1)->F2_EMISSAO)) == 9
									DGXLS->MES := "09"
								Else
									DGXLS->MES := "08"
								EndIf
							ElseIf (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "10" + StrZero(GETMV("DBM_DTFCKA"),2)) 
						...
						If MV_PAR01 == 4 
							If (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "11" + StrZero(GETMV("DBM_DTFCKA"),2))
								If Month(StoD((cAliSZ1)->F2_EMISSAO)) == 11
									DGXLS->MES := "11"
								Else
									DGXLS->MES := "10"
								EndIf	
							ElseIf (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)))  + "12" + StrZero(GETMV("DBM_DTFCKA"),2))
								If Month(StoD((cAliSZ1)->F2_EMISSAO)) == 12
									DGXLS->MES := "12"
								Else
									DGXLS->MES := "11"
								EndIf
							ElseIf (cAliSZ1)->Z1_DTENTRE <= (AllTrim(Str(Year(cl_PAR12)+1))  + "01" + StrZero(GETMV("DBM_DTFCKA"),2))
								DGXLS->MES := "12"
							EndIf
						EndIf 
...
cSqlDev += " AND F2_YCANAL BETWEEN '" + cGrpCli + "' AND '" + cGrpCli + "'"
...
PutSx1("DBM_KEYCOM","01","Quarter Base"	   		,"Quarter Base"			,"Quarter Base"  		,"mv_ch1","N",01,0,1,"G","(mv_par01 < 5)",""   ,"","","mv_par01","","","","","","","","","","","","","","","","",{},{},{})
PutSx1("DBM_KEYCOM","02","Ano Base"	   		,"Ano Base"			,"Ano Base"  		,"mv_ch2","C",04,0,1,"G","",""   ,"","","mv_par02","","","","","","","","","","","","","","","","",{},{},{})
//PutSx1("DBM_REGCOM","03","Canal de"			,"Canal de"	   		,"Canal de"	    	,"mv_ch3","C",06,0,1,"G","","ACA","","","mv_par03","","","","","","","","","","","","","","","","",{},{},{})
