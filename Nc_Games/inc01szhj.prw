...
If _cOpcao == "INCLUIR"
	...
	For _nI:=1 To nUsado
		...
		If AllTrim(Upper(aHeader[Len(aCols),2]))=="ZH_ITEM"
			aCols[1,Len(aCols)] := StrZero(Len(aCols),4)    // Soma1(_cItem,4)
		EndIf
...
Else
	...
	Do While !Eof() .And. SZH->ZH_CODIGO == _cCodigo
		...
		For _nI:=1 to nUsado
			If AllTrim(Upper(aHeader[_nI,2]))=="ZH_ITEM"
				aCols[Len(aCols),_nI] := StrZero(Len(aCols),4)    // Soma1(_cItem,4)
			EndIf
...
// ************************************************************************** // 
////////////////////////////////////////////////////////////////////////////////
User Function VAL_PRVSUG()
Private _lRet    := .T.
Private _nPrv0   := aCols[N][aScan(aHeader,{|x| AllTrim(Upper(x[2]))=="ZH_PRV0" })]
Private _nPrvSug := aCols[N][aScan(aHeader,{|x| AllTrim(Upper(x[2]))=="ZH_PRVSUG" })]

If _nPrv0 > 0
	If _nPRVSug <= 0
		Alert("Preco Sugerido Obrigatorio para Itens com Preco Zero...")
		_lRet := .F.
	EndIf
EndIf
If _lRet
	If _nPrvSug < _nPrv0
		Alert("Preco Sugerido NAO pode ser menor que preco zero...")
		_lRet := .F.
	EndIf
EndIf
Return(_lRet)
...
// ************************************************************************** // 
////////////////////////////////////////////////////////////////////////////////
User Function VAL_TABEMABERTO()
Private _aArea      := GetArea()
Private _cProduto   := aCols[N][aScan(aHeader,{|x| AllTrim(Upper(x[2]))=="ZH_PRODUTO" })]
...
If _lProdEmTab
	Alert("Produto encontrado na Tabela "+SZH->ZH_CODIGO+" que tem Status de Pendente Efetivacao...")
	_cProduto := Space(15)
EndIf
