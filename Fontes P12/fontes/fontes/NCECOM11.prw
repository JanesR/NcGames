...
User Function NCECOM11(lB2B)
	...
	Local lRetorno		:=.T.
	Local cGrupoVend	:=Alltrim(U_MyNewSX6("NCG_000113","000001","C","Grupo Vendedor","","",.F. ))

	Private cCadastro 	:= "Monitor - E-commerce"
	Private oBrowse
	Private lVendedor	:=Ascan(aGrupos,cGrupoVend)>0 //Vendedor
	Private aRotina 	:= xMenuDef(lB2B,lVendedor)
	Default lB2B:=.T.
	...
	AADD(aCpoZC5,"ZC5_VEND")
	AADD(aCpoZC5,"ZC5_NOMEVE")


	AADD(aCpoZC5,"ZC5_CONDPG")
	AADD(aCpoZC5,"ZC5_DCONDP")
	AADD(aCpoZC5,"ZC5_COND")
...
User Function NcEcomMon(cAlias, nRecno, nOpc)
	...
	Else
		cLabel:=Iif(AllTrim(ZC5->ZC5_COND)=='FAT',"Liberar Credito","Atualiza Reserva" )


		oButAtuCR	:= TButton():New( (aSize[4]*65/100)-35, aSize[3]-421,cLabel,oWin02,{ ||LJMsgRun("Aguarde o processamento...","Aguarde...",{|| AtuReserva(ZC5->ZC5_NUM, ZC5->ZC5_NUMPV, ZC5->ZC5_STATUS,ZC5->ZC5_PVVTEX) })}, 50,10,,,.F.,.T.,.F.,,.F.,,,.F. )
		...
		If AllTrim(ZC5->ZC5_COND)=='FAT'


			oButLibWMS:bAction := {|| alert("Pedido Faturado nใo ้ permitido Enviar WMS","NcGames")}

			If ZC5->ZC5_STATUS<>'08' .Or. lVendedor
				oButAtuCR:bAction := {||  alert("Pedido Faturado nใo ้ permitido "+cLabel,"NcGames")}			
			EndIf
...
Static Function VisDetPed(nPedSite, cNumPedProt,cPvVtex)
	...
	Local aCpoItPVSt	:= {"ZC6_NUM", "ZC6_ITEM", "ZC6_KIT","ZC6_PRODUT", "B1_XDESC","ZC6_QTDVEN", "B2_QATU", "ZC6_VLRUNI", "ZC6_VLRTOT", "B1_MSBLQL", "B1_BLQVEND" }
	Local aScreen 		:= GetScreenRes()
	Local nWStage 		:= 800
	Local nHStage 		:= 500
	Local oDlg
	...
	If !Empty(ZC5->ZC5_PVVTEX)
		...
		aCpoItPV   	:= {"ZC5_PVVTEX", "ZC5_NUMPV","ZC6_KIT", "C6_PRODUTO", "C6_DESCRI", "C6_QTDVEN", "C0_QTDPED", "C0_QTDPED","C6_PRCVEN", "C6_VALOR", "C6_PRCTAB", "C6_TES", "ZC5_VDESCO", "ZC5_PDESCO"}
	...
	If (	Len(aAcolsCbPV) > 0)  .Or. (Len(aAcolsPVSt) > 0)
		...
		If !Empty(alltrim(cNumPedProt))//Resumo do pedido no Protheus
...
฿฿#฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
Static Function EnvStsCan(nPVSite,cNumPVP, cNota,cPVVtex,cMensagem,lAuto)
	...
	If Empty(cNota)
		...
		If Alltrim(ZC5->ZC5_STATUS) != "90"
			...
			If VldCancel(nPVSite, cNumPVP)//Valida็ใo do cancelamento
				...
				If lAuto .Or. Aviso("Cancelamento","Nใo existe nota fiscal para este pedido "+IIf(!Empty(cPVVtex),cPVVtex,Alltrim(Str(nPVSite)))+". Deseja realmente cancelar ? ",{"Sim","Nใo"},2 ) == 1

					//Tela para justificativa
					If !Empty(cMensagem)
						cMsgJustif :=cMensagem
					Else
					...
					If !Empty(cMsgJustif)
						...
						If Ec11Canc(cNumPVP)
							...
							If  AllTrim(ZC5->ZC5_PAGTO)<>'FAT' .And. !VlDExisRA(ZC5->ZC5_NUM) .And. Alltrim(ZC5->ZC5_PAGTO) == "2"
...
Static Function CriaACbPed(cNumPedProt,cPvVtex)
	...
	cQuery   += "  AND SC5.C5_NUM = '"+alltrim(cNumPedProt)+"' "
...
Static Function CriaAItSt(nPvSite, cArmPad,cPvVtex)
	...
	If !Empty(cPvVtex)
		cQuery   := " SELECT ZC6_NUM, ZC6_ITEM,ZC6_KIT, ZC6_IDPROD,ZC6_PRODUT,ZC6_DESCRI, B1_XDESC, ZC6_QTDVEN, ZC6_VLRUNI, ZC6_VLRTOT, B1_XDESC, B1_MSBLQL, "
	...
	Else
		...
		cQuery   := " SELECT ZC6_NUM, ZC6_ITEM, ZC6_KIT, ZC6_IDPROD,ZC6_DESCRI,ZC6_PRODUT, B1_XDESC, ZC6_QTDVEN, ZC6_VLRUNI, ZC6_VLRTOT, B1_XDESC, B1_MSBLQL, "
		cQuery   += "         B1_BLQVEND, (B2_QATU - B2_RESERVA - B2_QEMP) AS SALDO FROM "+RetSqlName("ZC6")+" ZC6 "
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
/*/
Static Function xMenuDef(lB2B,lVendedor)
...
Static Function ManClient(cCodCli, cLoja)
	...
	If !Empty(cCodCli) .And. !Empty(cLoja)
		...
		If SA1->(DbSeek(xFilial("SA1") + cCodCli + cLoja ))
			...
			If lVendedor
				nOpc := Aviso("Cadastro de Cliente","O que deseja fazer no cadastro de cliente ? ",	{"Visualizar","Sair"})
			Else 
				nOpc := Aviso("Cadastro de Cliente","O que deseja fazer no cadastro de cliente ? ",	{"Visualizar","Alterar","Sair"})
			EndIf

			If lVendedor .And. nOpc==2
				nOpc:=3 // Sair
			EndIf	
...
Static Function AtuReserva(nPedSite, cNumPed, cStatus,cPvVtex)
	...
	If !Empty(cNumPed) .And. (Alltrim(cStatus) $ "4|5|10|08" ) .And. (!VeriEnvWMS(cNumPed))

		DbSelectArea("SC5")
		DbSetOrder(1)
		If SC5->(DbSeek(xFilial("SC5") + cNumPed  ))

			cMensagem:="Deseja atualizar a reserva ?"
			If cStatus=='08'
				cMensagem:="Deseja liberar  Pedido Faturado de Prevenda para analise credito ?"
				SC9->(DbSetOrder(1))
				If Aviso("Alerta",cMensagem, {"Sim","Nใo"}) == 1
					lJaLiberado:=.F.
					...
					If lJaLiberado
						MsgInfo("Pedido Venda jแ liberado no cr้dito.")
					Else
						MaAvalSC5( 'SC5', 9 )		
						...
						If SC9->(DbSeek(xFilial("SC9")+SC5->C5_NUM ))
							...
							SC5->(MsUnLock())
						EndIf
						cLog := "Pedido PREVENDA Faturado enviado para Analise de Credito."
					EndIf
					//=====================================================================================================================================
					//==Adicionado envio de Workflow quando o pedido for enviado para a libera็ใo do cr้dito
					//====================================================================================================================================
							if !Empty(cLog) .And. cLog != Nil
								DbSelectArea("ZC5")
								DbSetOrder(2)
								if DbSeek(xFilial("ZC5")+SC5->C5_NUM)
									oFunc := FuncoesDiversas():New()
									
									oFunc:setHtml('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">')
									oFunc:setHtml(oFunc:getHtml()+'<html xmlns="http://www.w3.org/1999/xhtml">    ')
									oFunc:setHtml(oFunc:getHtml()+'    <head>'	)
									oFunc:setHtml(oFunc:getHtml()+'		   <body>')

										cAssunto := "PV E-Commerce Faturado " + AllTrim(Str(ZC5->ZC5_NUM)) + "\"+ AllTrim(ZC5->ZC5_NUMPV) +"."
										oFunc:setHtml(oFunc:getHtml()+'			<span> ' + cLog + ' </span>')					
										
									oFunc:setHtml(oFunc:getHtml()+'			</body>')
									oFunc:setHtml(oFunc:getHtml()+'</html>	')
										
									If !oFunc:enviaEmail(cAssunto,,oFunc:retornaEmailVendedorPedido(AllTrim(ZC5->ZC5_NUMPV)),)//cAssunto, aAnexos, cEmailTo, cEmailCc o corpo foi setado no metodo oFunc:setHtml
											U_NCECOM09(ZC5->ZC5_NUM, ZC5->ZC5_NUMPV,'Erro ao enviar e-mail para '+cEmailTo,oFunc:getErro(),ZC5->ZC5_STATUS,.T.,,,ZC5->ZC5_PVVTEX)
									EndIF
								...
								DbCloseArea("ZC5")
							End if
				U_NCECOM09(nPedSite, cNumPed,IIf(cStatus=='08', "Pedido Faturado de Prevenda para analise credito","Atualiza็ใo de reserva")," Log da atualiza็ใo: "+CRLF+cMsgLog,"",.T.)	
			ElseIf Aviso("Alerta",cMensagem, {"Sim","Nใo"}) == 1
...
Static Function NCECFL11(cOpc,cLoja,lB2B,lPgtoBol,aParametros)
	...
	If lVendedor
		aAreaSA3:=SA3->( {Recno(),IndexOrd()} )
		SA3->(DbSetOrder(7))//A3_FILIAL+A3_CODUSR

		If SA3->(DbSeek(xFilial("SA3")+__cUserId  ))
			cCodVend:=SA3->A3_COD
		Else
			cCodVend:="NAO_ENCONTRADO"
		EndIf

		SA3->(DbSetOrder(aAreaSA3[2]))
		SA3->(DbGoTo(aAreaSA3[1]))


		cFiltro  	+= " AND ZC5_VEND ='"+cCodVend+"'"
	Endif
...
Static Function PergFil(aParams,lB2B,lPgtoBol)
	...
	If !lB2B
		...
		AADD(aLojas,"10=MARKETPLACE FNAC")
...
User Function PvCancel(nZC5Rec,lCancel,cMotivo)
...
User Function COM11Est(cMensagem,cPedido)
	Local aAreaAtu:=GetArea()
	Local cAliasQry
	...
	If !Empty(cPedido)
		...
		SELECT ZC5.R_E_C_N_O_ RECNOZC5 
		FROM %Table:ZC5% ZC5
		WHERE ZC5.ZC5_FILIAL = %xFilial:ZC5%		
		AND ZC5.ZC5_NUMPV =%Exp:cPedido%
		AND ZC5.ZC5_STATUS != '90'
		AND ZC5.ZC5_ESTORN != 'S'
		AND ZC5.D_E_L_E_T_=' '

		EndSql

		If (cAliasQry)->RECNOZC5>0
			ZC5->(DbGoTo((cAliasQry)->RECNOZC5))
			lPosicionado:=.T.
		EndIf
	...
	If lPosicionado
		EnvStsCan(ZC5->ZC5_NUM, ZC5->ZC5_NUMPV, ZC5->ZC5_NOTA,ZC5->ZC5_PVVTEX,cMensagem,.T.)
	EndIf	

	RestArea(aAreaAtu)
Return	

User Function COM11DPG()
	Local aAreaAtu:=GetArea()
	Local aAreaSC5:=SC5->(GetArea())
	...
	If !Empty(ZC5->ZC5_NUMPV) .And. SC5->( MsSeek(xFilial("SC5")+ZC5->ZC5_NUMPV  ) ) .And. SE4->( MsSeek( xFilial("SE4")+SC5->C5_CONDPAG  ) )
		cDescricao:=SE4->E4_CODIGO+"-"+AllTrim( SE4->E4_DESCRI )
	EndIf
