...
User Function XmlNfeSef(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
			...
			If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
				...
				Else
					...
					Else
						...
						aadd(aDest,SA2->A2_TIPO)//Posição 20 //Alterado 21/07/2016
					...
					SF2->F2_DOC == (cAliasSD2)->D2_DOC
						...
						If lContinua
							...
							//Específico NCGAMES
							cDescProd := AllTrim(SB1->B1_XDESC)+iif(Val(SB1->B1_CODBAR)!=0," - Cod.Barra: "+AllTrim(SB1->B1_CODBAR),"") //IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)
							SA7->(dbSetOrder(1))
							If SA7->(MsSeek( xFilial("SA7") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) ))
								cCodProd  := ALLTRIM(SA7->A7_CODCLI)
								cDescProd := alltrim(SA7->A7_DESCCLI)+iif(Val(SB1->B1_CODBAR)!=0," - Cod.Barra: "+AllTrim(SB1->B1_CODBAR),"")
							EndIf
							//BLOCO RETIRADO POR RODRIGO 01/06/2012		
							...
							aadd(aICMUFDest,{})
							aadd(aIPIDevol,{})
							//aadd(aPedCom,{}) -- NCGAMES
							If !Empty(AllTrim(SC6->C6_PEDCLI))
								aadd(aPedCom,{AllTrim(SC6->C6_PEDCLI),(cAliasSD2)->D2_ITEM})
							Elseif !Empty(AllTrim(SC5->C5_PEDCLI))
								cPedCli:=AllTrim(SC5->C5_PEDCLI)
								If SC5->C5_XECOMER=="C" .And. (nAt:=At("(",cPedCli))>0
									cPedCli:=Left(cPedCli,nAt-1)
								EndIf
								aadd(aPedCom,{cPedCli,(cAliasSD2)->D2_ITEM})
							Else
								aadd(aPedCom,{})
							Endif
							aadd(aPisAlqZ,{})
		...
		If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				Else
					...
					aadd(aDest,SA2->A2_DDD+SA2->A2_TEL)
					If !Upper(SA2->A2_EST) == "EX"
						aadd(aDest,VldIE(SA2->A2_INSCR)) //14
					Else
						aadd(aDest,"")
					EndIf
					aadd(aDest,"")//SA2->A2_SUFRAMA
					...
					aAdd(aDest, "")// Posição 18 (referente a A1_IENCONT, sendo passado como vazio já que não existe A2_IENCONT)
					aadd(aDest,SA2->A2_INSCRM)
					aadd(aDest,SA2->A2_TIPO)//Posição 20
					aadd(aDest,SA2->A2_PFISICA)//21-Identificação estrangeiro
	...
	IF ExistBlock("PE01NFESEFAZ")
		...
		aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aRefECF}
...
Static Function NfeEmit(aIEST, cVerAmb, aDest)
	...
	Local cFoneNew		:=  Alltrim(U_MyNewSX6("NCG_000091","1140953001","C","Tel. que será utilizado na Danfe","","",.F. ))
	Local cFoneB2C		:=  Alltrim(U_MyNewSX6("EC_NCG0029","1140953005","C","Tel. que será utilizado nos pvs B2C ","","",.F. ))
	...
	*/
	...
	If cCanal == "990001" //Canal Ecommerce B2C
		cString += NfeTag('<fone>',cFoneB2C)
	Else
		cString += NfeTag('<fone>',cFoneNew)
	EndIf
	//cString += NfeTag('<fone>',cFoneDest) //Padrão
	cString += '</enderEmit>'
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,aNota,aICMSZFM,aDest,cIpiCst,aFCI,lIcmDevol,nVicmsDeson,nVIcmDif,cMunPres,aAgrPis,aAgrCofins,nIcmsDif,aICMUFDest,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,cAmbiente,aIPIDevol,nvBCUFDest)
	...
	If cVerAmb== "3.10"
		//cString += '<vUnCom>'+ConvType(aProd[16],21,8)+'</vUnCom>'    -- modificado especifico ncgames - considerr aumento do tamanho das casas decimais do valor unitário do SD1	
	...
	EndIf
	If Len(aICMUFDest) > 0 .and. (cIdDest == "2" .and. cIndFinal == "1" .and. cIndIEDest == "9") .and. (Len(aISSQN)== 0) .and. (cAmbiente == "2" .or. (cAmbiente == "1" .and. Val(FsDateConv(aNota[03],"YYYY")) >= 2016)) .and. (iif(Len(aComb) > 0 .And. !Empty(aComb[01]),aComb[01] $ NfeCodANP(),.T.))
		cString += '<imposto>'
	...
	Elseif Len(aICMUFDest) == 0 .and. (cIdDest == "2" .and. cIndFinal == "1" .and. cIndIEDest == "9") .and. (Len(aISSQN)== 0) .and. (cAmbiente == "2" .or. (cAmbiente == "1" .and. Val(FsDateConv(aNota[03],"YYYY")) >= 2016)) .and. (iif(Len(aComb) > 0 .And. !Empty(aComb[01]),aComb[01] $ NfeCodANP(),.T.))
			...
			*/
			...
			Else
				If val(aProd[27]) < 500
					...
					If DbSeek(xFilial("SD1") + DtoS(aNota[03]) + aNota[02] + aNota[01])
						DbSelectArea("SF2")
						DbSetOrder(1)
						
						If DbSeek(xFilial("SF2") + SD1->D1_NFORI + SD1->D1_SERIORI + SD1->D1_FORNECE + SD1->D1_LOJA)
							cAnoDevol := SF2->F2_EMISSAO
						EndIf
						DBCloseArea("SF2")
					EndIf
					DbCloseArea("SD1")
				Else
					...
					If DbSeek(xFilial("SD2") + aNota[02] + aNota[01])
						DbSelectArea("SF1")
						DbSetOrder(1)
						
						If DbSeek(xFilial("SF1") + SD2->D2_NFORI + SD2->D2_SERIORI + SD2->D2_CLIENTE + SD2->D2_LOJA)
							cAnoDevol := SF1->F1_EMISSAO
						EndIf
						DBCloseArea("SF1")
					EndIf
					DBCloseArea("SD2")
			...
			IF !Empty(aNota) .and. (nPos := aScan(aPPDifal,{|x| x[1]== Year(cAnoDevol)})) > 0// Verifica o ano da nota vinculada para pegar a aliquota do parâmetro
				aPICMSInter:= aPPDifal[nPos][2]
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,aRefECF,nVicmsDeson,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,nvBCUFDest,aICMUFDest)
	...
	//Modificação por Jorge Heitor em 26/02/2014 - Chamado 003138
	//Mensagem personalizada para o NCM 
	If cTipo == "1" //Somente Notas Fisccais de Saida.

		_aNCM := {}
		_cNCM := ""
		_cTmp := ""

		// Le os NCM's do array pre existente de produtos
		For x:= 1 to Len(aProd)
			_cTmp := _BuscaNcm(aProd[x])
			If !Empty(_cTmp)
				If aScan(_aNCM,{|y| y[1] == _cTmp}) == 0
					aAdd(_aNCM,{_cTmp})
				EndIf
			EndIf
		Next x
		...
		If Len(_aNCM) > 0
			...
			cQuery := "SELECT DISTINCT YD_XMSGNFS AS MSG FROM " + RetSqlName("SYD") + " WHERE YD_FILIAL = '"+xFilial("SYD")+"' AND YD_TEC IN ("+_cNCM+") "
			cQuery := ChangeQuery(cQuery)

			If Select("TNCM") > 0 ; TNCM->(dbCloseArea()) ; EndIf

			TcQuery cQuery ALIAS "TNCM" NEW

			dbSelectArea("TNCM")

			While !Eof()
				cString += ConvType(AllTrim(TNCM->MSG),Len(AllTrim(TNCM->MSG)))+". "
				dbSkip(1)
			End
	...
	Else
		...
		If !Empty(cPercTrib) .And. nTotTrib > 0
			If SA1->A1_GRPTRIB $ "CFI/CFS/CZF" .OR. (!SA1->A1_GRPTRIB $ "CFI/CFS/CZF" .AND. SA1->A1_SATIV1 == "000103")
				//vai mensagem somente caso o cliente for consumidor final - Específico NCGAMES
...
Static Function RetPrvUnit(cAliasSD2,nDesconto)
	...
	Else
		If !(lMvNFLeiZF)
			If ((cAliasSD2)->D2_QUANT < 1) .OR. ((cAliasSD2)->D2_PRCVEN < 1)    
				...
				elseif (cAliasSD2)->D2_PRUNIT < (cAliasSD2)->D2_PRCVEN .OR.  (cAliasSD2)->D2_DESCZFR  > 0 .OR. (TAMSX3("D2_PRCVEN")[2] > TAMSX3("D2_TOTAL")[2]) 	
...
±±ºPrograma  ³NFESEFAZ  ºAutor  ³Microsiga           º Data ³  03/24/14   º±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function _BuscaNcm(_aProduto)

	Local cNCM := ""
	Local aArea := GetArea()
	Local cCodPro := _aProduto[2]
