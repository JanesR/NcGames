...
oDlg1      := MSDialog():New( 091,232,247,521,"Importação de Notas em Excel",,,.F.,,,,,,.T.,,,.T. )
...
oSay2      := TSay():New( 012,112,{||"Série:"},oDlg2,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,020,008)
oSay3      := TSay():New( 012,216,{||"Emissão:"},oDlg2,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,032,008)
...
While !FT_FEOF()  //FACA ENQUANTO NAO FOR FIM DE ARQUIVO
   ...
   aItens := strtokarr(cBuffer,";")
	...
	IF len(aItens) < 12
		alert("Algum dos Campos da Planilha está em não foi preenchido. Favor verificar e reprocessar.")
		return
	else 
...
EndIF
 ...
 IF(Empty(SB1->B1_COD))
	 cNotProd+= "Item: "+aArrayItens[ny][1]+" Cód.:"+ aArrayItens[ny][4]+ "-"	+aArrayItens[ny][5]+CRLF
 EndIf 
 dbCloseArea("SB1")
 
next   
IF(!cNotProd =="")
	alert("Os seguintes produtos não forma encontrados no cadastro de produtos:"+CRLF+cNotProd)  
	return
EndIf
...
±±ºPrograma  ³NCONVVAL ºAutor  ³Microsiga           º Data ³ 07/04/15    º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Retira pontuação do CNPJ           º±±
±±º          ³                   º±±
