...
*/
user Function PrTRFDanf(cEmpNota,cFilNota,cNota,cSerieNf,cCliente,cCliLoja,cNome,cTPIMP,cPrinter,lViewPDF,lSavePDF)
				...
				If lPrinter
					...
					*/
					...
					do Case
						...
						oPrinter2	:= FWMSPrinter():New(cNameArq,IMP_SPOOL,lAdjustToLegacy,cDirDanfe + cEndArq,.T.,.F.,Nil,cPrinter,.T.,.F.,.F.,.F.,001)
						case cTPIMP == "3"
						oPrinter	:= FWMSPrinter():New(cNameArq,IMP_PDF,lAdjustToLegacy,/*cPathInServer*/,lDisableSetup,.F.,Nil,/*cPrinter*/,.T.,.F.,.F.,lViewPDF)
						oPrinter2	:= FWMSPrinter():New(cNameArq,IMP_SPOOL,lAdjustToLegacy,cDirDanfe + cEndArq,.T.,.F.,Nil,cPrinter,.T.,.F.,.F.,.F.,001)
					endCase
