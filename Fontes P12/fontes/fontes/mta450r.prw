...
±±ºPrograma  ³ MTA450R   ºAutor  ³Rodrigo Okamoto    º Data ³  11/10/12   º±±
...
If DbSeek(xFilial("SZR")+SC9->C9_PEDIDO)
	While !eof() .and. SC9->C9_PEDIDO == SZR->ZR_PEDIDO
		If SZR->ZR_TIPO == "2" .AND. SZR->ZR_DTOCORR == DDATABASE
			//se já foi registrada a rejeição, não abrirá novamente a tela para digitação do motivo
			RestArea(aArea)
			Return
		EndIf
		SZR->(DbSkip())
...
@ 100,001 To 550,800 Dialog oDlg Title "Rejeição do crédito do cliente: " + SA1->A1_NOME + " " + cCliAtu+" / "+cLojaAtu
//@ 003,010 To 375,500
@ 014,014 Say OemToAnsi("Descreva o motivo da rejeição do crédito")
