...
clQry += " 	GROUP BY ACA_GRPREP, "+CRLF
clQry += " 	         ACA_DESCRI	 "+CRLF
clQry += " 	ORDER BY ACA_GRPREP, "+CRLF
clQry += " 	         ACA_DESCRI	 "+CRLF
...
While (clAlia1)->(!EOF())
	...
	aAdd(aFat,{(clAlia1)->(ACA_DESCRI),nTotalFat})
	
	cDescri2	+= cCanDescri + "R$ " + AllTrim(Transform((clAlia1)->TOTAL,"@E 999,999,999.99")) + CRLF
	cHtml2		+= '<tr><td width="70%">'+ cCanDescri +'</td>'
...
cSqlEft += "	GROUP BY ACA_GRPREP, ACA_DESCRI "+CRLF
cSqlEft += "	UNION "+CRLF
...
//
cSqlEft += "	GROUP BY ACA.ACA_GRPREP, ACA.ACA_DESCRI "+CRLF
cSqlEft += "	UNION "+CRLF
cSqlEft += "	SELECT 'FAT' AS TIPO, "+CRLF
cSqlEft += "		ACA_GRPREP GRUPO, "+CRLF
cSqlEft += "		ACA_DESCRI DESCR, "+CRLF
...
cQryAb := " "
cQryAb += " SELECT DISTINCT C5_YCANAL, "+CRLF
...
cQryAb += " GROUP BY C5_YCANAL," +CRLF
cQryAb += " 		 ACA_DESCRI" +CRLF
cQryAb += " ORDER BY C5_YCANAL" +CRLF
...
While (cAliaAb)->(!EOF())
	...
	cDescri3	+= cCanDescri + "R$ " + AllTrim(Transform((cAliaAb)->TOTAL,"@E 999,999,999.99")) + CRLF
	cHtml3		+= '<tr><td width="70%">'+ cCanDescri +'</td>'
...
While (cAliaAb)->(!EOF())
	...
	cDescri4	+= cCanDescri + "R$ " + AllTrim(Transform((cAliaAb)->TOTAL,"@E 999,999,999.99")) + CRLF
	cHtml4		+= '<tr><td width="70%">'+ cCanDescri +'</td>'
...
While (cSqlAlias)->(!EOF())
	...
	cDescri5	+= cCanDescri + "R$ " + AllTrim(Transform((cSqlAlias)->NVLRTOT,"@E 999,999,999.99")) + CRLF
	cHtml5		+= '<tr><td width="70%">'+ cCanDescri +'</td>'
...
While (cSqlAlias)->(!EOF())
	...
	cDescri6	+= cCanDescri + "R$ " + AllTrim(Transform((cSqlAlias)->NVLRTOT,"@E 999,999,999.99")) + CRLF
	cHtml6		+= '<tr><td width="70%">'+ cCanDescri +'</td>'
...
For nI := 1 to Len( aAnalise)
	...
	nToRejeit	+= aAnalise[nI][2]
	cCanDescri	:= "E-commerce "
	
	cDescri6	+= cCanDescri + "R$ " + AllTrim(Transform(aAnalise[nI][2],"@E 999,999,999.99")) + CRLF
	cHtml6		+= '<tr><td width="70%">'+ cCanDescri +'</td>'
	cHtml6		+= '<td width="5%">R$</td>'
	cHtml6		+= '<td width="25%"><div align="right">'+ AllTrim(Transform(aAnalise[nI][2],"@E 999,999,999.99")) +'</div></td></tr>'
...
While (cSqlAlias)->(!EOF())
	...
	cDescri7	+= cCanDescri + "R$ " + AllTrim(Transform((cSqlAlias)->NVLRTOT,"@E 999,999,999.99")) + CRLF
	cHtml7		+= '<tr><td width="70%">'+ cCanDescri +'</td>'
...
For nI := 1 to Len( aAnalise)
	...
	nToAnalis	+= aAnalise[nI][2]
	cCanDescri	:= "E-commerce "
	
	cDescri7	+= cCanDescri + "R$ " + AllTrim(Transform(aAnalise[nI][2],"@E 999,999,999.99")) + CRLF
	cHtml7		+= '<tr><td width="70%">'+ cCanDescri +'</td>'
	cHtml7		+= '<td width="5%">R$</td>'
	cHtml7		+= '<td width="25%"><div align="right">'+ AllTrim(Transform(aAnalise[nI][2],"@E 999,999,999.99")) +'</div></td></tr>'
...
cHtml += '<div id="principal" style="margin: 20px auto; width: 80%; padding: 15px; background-color: #FFF; font-family: Tahoma; font-size: 12px;" >
...
cQuery+= " 	AND SC5.C5_TIPO = 'N' "+CRLF  //Retirar Canais
...
cQuery+= " 	GROUP BY C5_YCANAL, "+CRLF
cQuery+= "           ACA_DESCRI "+CRLF
cQuery+= " 	ORDER BY C5_YCANAL "+CRLF
...
cQry += " SELECT ZC5_STATUS, SUM(C6_VALOR) total "+ CRLF
...
While (cAliasQry)->(!Eof())
	...
	(cAliasQry)->(aAdd(aAnaliCre,{ZC5_STATUS, TOTAL}))
...
cQuery:="SELECT C5_FILIAL,C5_NUM,ACA_DESCRI,SC9.C9_QTDLIB,SC6.C6_PRCVEN FROM SC5010 SC5 LEFT OUTER JOIN ACA010 ACA ON ACA.D_E_L_E_T_ = ' ' AND ACA.ACA_GRPREP = SC5.C5_YCANAL LEFT OUTER JOIN SC6010 SC6 ON SC6.D_E_L_E_T_ = ' ' AND SC6.C6_FILIAL = SC5.C5_FILIAL AND SC6.C6_NUM = SC5.C5_NUM AND SC6.C6_CLI = SC5.C5_CLIENTE AND SC6.C6_LOJA = SC5.C5_LOJACLI AND (SC6.C6_BLQ <> 'R'      OR SC6.C6_QTDENT <>0) LEFT OUTER JOIN SC9010 SC9 ON SC9.D_E_L_E_T_ = ' ' AND SC9.C9_FILIAL = SC6.C6_FILIAL AND SC9.C9_PEDIDO = SC6.C6_NUM AND SC9.C9_CLIENTE = SC6.C6_CLI AND SC9.C9_LOJA = SC6.C6_LOJA AND SC9.C9_PRODUTO = SC6.C6_PRODUTO AND (SC9.C9_BLEST = ' '     OR SC9.C9_BLEST = '10') AND (SC9.C9_BLWMS = '02'      OR SC9.C9_BLWMS = ' ') WHERE SC5.C5_FILIAL = '03'   AND SC9.C9_DATALIB >= '20141116'   AND SC9.C9_DATALIB <= '20141231'   AND SC9.C9_BLCRED = ' '  ORDER BY 1,2"
cSqlAlias:="sc9pv"
...
While (cSqlAlias)->(!EOF())
	...
	cHtml7		+=(cSqlAlias)->(C5_NUM+";"+ACA_DESCRI+";"+Transform(C9_QTDLIB,"@E 999,999,999.99")+";"+Transform(C6_PRCVEN,"@E 999,999,999.99"))+CRLF
