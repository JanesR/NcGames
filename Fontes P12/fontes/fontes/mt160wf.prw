...
±±³Funcao    ³ MT160WF  ³ Autor ³ CARLOS N. PUERTA      ³ Data ³ OUT/2012 ³±±
...
Private _aAreaC7   := {}
Private _aAreaAK   := {}
Private _cPedido   := SC7->C7_NUM
Private _cUserLib  := Space(06)
Private _cMailLib  := ""
Private _cNome     := UsrRetName(SC7->C7_USER)
Private _cFornece  := SC7->C7_FORNECE
Private _cLoja     := SC7->C7_LOJA
Private _cNomeFor  := POSICIONE("SA2",1,XFILIAL("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA,"A2_NOME")
...
Do While !Eof() .And. SC7->C7_NUM == _cPedido
	...
	If !EMPTY(_cUserLib)  .And. (_cImport <> "S" .Or. _cTipo = "MC")
        ...
        dbSelectArea("SC8")
        dbSetOrder(8)
        If dbSeek(xFilial("SC8")+SC7->C7_PRODUTO,.T.)
            Do While !Eof() .And. SC7->C7_PRODUTO == SC8->C8_PRODUTO
                If Empty(_cCot001)
                    _cCot001 := SC8->C8_NUM
                    _nVal001 := SC8->C8_PRECO
                Else
                    If Empty(_cCot002)
                        _cCot002 := SC8->C8_NUM
                        _nVal002 := SC8->C8_PRECO
                    Else
                        If Empty(_cCot003)
                            _cCot003 := SC8->C8_NUM
                            _nVal003 := SC8->C8_PRECO
                        Else
                            _cCot001 := _cCot002
                            _cCot002 := _cCot003
                            _cCot003 := SC8->C8_NUM
		...
		If dbSeek(xFilial("SZP")+SC7->C7_NUM+SC7->C7_ITEM)
            ...
            SZP->ZP_UCOT001 := _cCot001
            SZP->ZP_UCOT002 := _cCot002
            SZP->ZP_UCOT003 := _cCot003
            SZP->ZP_UVALC01 := _nVal001
            SZP->ZP_UVALC02 := _nVal002
            SZP->ZP_UVALC03 := _nVal003
		Else
			RecLock("SZP",.T.)
