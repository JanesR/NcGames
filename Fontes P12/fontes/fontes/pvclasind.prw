...
±±³Fun‡…o    ³ PVCLASIND ³ Autor ³ ERICH BUTTNER	      ³ Data ³16.06.10 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄ±±
±±³Descri‡…o ³ Pick-List Classificação indicativa - NC Games               ³±±
...
/*/
User Function PVCLASIND(cMarca)

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Define Variaveis                                             ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ

LOCAL wnrel     := "PVCLASIND"
LOCAL tamanho	:= "G"
LOCAL titulo    := OemToAnsi("Classificação indicativa (Expedicao)")
LOCAL cDesc1    := OemToAnsi("Emiss„o de produtos a serem separados pela expedicao, para")
LOCAL cDesc2    := OemToAnsi("etiquetagem da classificação indicativa")
LOCAL cDesc3	:= ""
LOCAL cString	:= "SC6"
LOCAL cPerg  	:= ""

PRIVATE aReturn         := {"Zebrado", 1,"Administracao", 2, 2, 1, "",0 }
PRIVATE nomeprog	:= "PVCLASIND"
PRIVATE nLastKey 	:= 0
PRIVATE nBegin		:= 0
PRIVATE aLinha		:= {}
PRIVATE li			:= 80
...
titulo := OemToAnsi("Lista p/ Etiqueta - Classificação Indicativa")
...
#IFDEF TOP
	...
	cQuery += "FROM SC6010 SC6 ,SC5010 SC5, SC9010 SC9,SB1010 SB1 "
	cQuery += "WHERE SC6.C6_QTDVEN > 0 AND "
	cQuery += "SC6.C6_NUM = SC5.C5_NUM AND "
	cQuery += "SC9.C9_PEDIDO = SC6.C6_NUM AND SC9.C9_ITEM = SC6.C6_ITEM AND "
	cQuery += "SC6.C6_FILIAL = SC5.C5_FILIAL AND SC9.C9_BLEST <> '02' AND " //SC9.C9_BLEST = '  ' AND "
...
// 	IF EMPTY(POSICIONE("SC9",1,XFILIAL("SC9")+(cAliasSC6)->C5_NUM+(cAliasSC6)->C6_ITEM,"C9_BLEST"))	
	IF POSICIONE("SC9",1,XFILIAL("SC9")+(cAliasSC6)->C5_NUM+(cAliasSC6)->C6_ITEM,"C9_BLEST") <> "02"
...
/*		IF DBSEEK(XFILIAL("SC5")+(cAliasSC6)->C5_NUM)
	...
	IF LI == 8
		...
		@ 9,180 Psay "PEDIDO : "+(cAliasSC6)->C5_NUM+" ->"+"PARTE: "+TRANSFORM ((cAliasSC6)->C9_SEQCAR, "@R 9")+" / "+TRANSFORM(SC9->C9_ULTIMCA, "@R 9")+IIF(ALLTRIM((cAliasSC6)->C5_REPICK) == "R"," - REIMPRESSO"," ")
