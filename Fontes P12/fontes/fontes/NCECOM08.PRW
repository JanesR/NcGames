...
If cAcao=="GRAVA_PEDIDO"
	...
	Do While (clAlias)->(!Eof())
		...
		lFaturado:=AllTrim(ZC5->ZC5_COND)=='FAT'
		...
		Else
			...
			cTransp	:= "000002"
			If lFaturado
				cCondPag:=M008CONPG(cCanal,"CIASHOP")
			EndIf
		EndIf
		...
		EndIf
			aadd(aCabec,{"C5_XCIAFAT"   	,IIf(lFaturado,"S","N")						,Nil})
		If Empty(cCondPag) .And. lWMPedido
			cCondPag:=M008CONPG(cCanal,"WM")
		EndIf
		...
		Do While (clAlias)->(!Eof() ) .And. Eval(bWbile)
			...
			ZC6->(DbGoTo( (clAlias)->RecZC6 ))
				SB5->(MsSeek(xFilial("SB5")+ZC6->ZC6_PRODUT)) 
			If cPreVenda=="N" .And. SB5->B5_YPREVEN=="S"
					cTipoLib :="2"
					cPreVenda:="S"
			EndIf
				If Ascan(aMesclado,SB5->B5_YPREVEN)==0
					AAdd(aMesclado,SB5->B5_YPREVEN)
				EndIf	
		...
		Else
			...
			If lWMPedido
				...
				If !lZerado .And. !cPreVenda=="S"
					If lBloqCred
						cLog := "Pedido em analise de Crédito."
						U_NCECOM09(ZC5->ZC5_NUM, ZC5->ZC5_NUMPV,cLog,cMsg,ZC5->ZC5_STATUS,lMsErroAuto,,,ZC5->ZC5_PVVTEX)
					Else
			...
			Else
				...
				EndIf
						...
						If lFaturado
							SA1->(DbSetOrder(1))
							If SA1->(DbSeek(xFilial("SA1")+SC5->(C5_CLIENTE+C5_LOJACLI) ) ) .And. ( SA1->A1_YBLQFIN=="1"  .Or. SA1->A1_LC==0   )  
							cLog:= Dtoc(MsDate())+"-"+Time()+" Cliente sem Credito na Ncgames"							
								U_COM11Est(cLog)
							ElseIf Len(aMesclado)>1
								cLog := Dtoc(MsDate())+"-"+Time()+" Itens mesclados pre-venda e não pre-venda"
								U_COM11Est(cLog)
								
							Else//If aMesclado[1]=="S"//Pre-Venda
								ZC5->(RecLock("ZC5",.F.))
								ZC5->ZC5_STATUS:='08'
								ZC5->(MsUnLock())
								If cPreVenda=="N"
									cLog :=	"Pedido Faturado catalogo enviado para Analise de Credito"
									U_NCECOM09(ZC5->ZC5_NUM, ZC5->ZC5_NUMPV,cLog," Log da atualização: Envio ao credito","",.T.)
								ElseIf cPreVenda=="S"
									cLog :=	"Pedido PREVENDA Faturado aguardando liberação para vendas"
									U_NCECOM09(ZC5->ZC5_NUM, ZC5->ZC5_NUMPV,cLog," Log da atualização: Envio ao credito","",.T.)
								EndIf
							EndIf
						EndIf
	...
	EndDo
			...
			//=====================================================================================================================================
			//==Adicionado envio de Workflow quando a condição de pagamento do pedido for igual á "FAT"
			//====================================================================================================================================
			IF AllTrim(ZC5->ZC5_COND) == "FAT" .And. AllTrim(ZC5->ZC5_CODPAG) == "54" .And. AllTrim(ZC5->ZC5_NUMPV) != ""
				...
				oFunc:setHtml(oFunc:getHtml()+'		   <body>')
					...
					//cAssunto := "Pedido faturado " + AllTrim(Str(ZC5->ZC5_NUM)) + "."
					...
					if !Empty(cLog) .And. cLog != Nil
						...
						oFunc:setHtml(oFunc:getHtml()+'			<span> Pedido gravado com sucesso, numero do pedido Protheus' +AllTrim(ZC5->ZC5_NUMPV)+ ' </span>')
					end if
				...
				If !oFunc:enviaEmail(cAssunto,,oFunc:retornaEmailVendedorPedido(AllTrim(ZC5->ZC5_NUMPV)),)//cAssunto, aAnexos, cEmailTo, cEmailCc o corpo foi setado no metodo oFunc:setHtml
						U_NCECOM09(ZC5->ZC5_NUM, ZC5->ZC5_NUMPV,'Erro ao enviar e-mail para '+oFunc:retornaEmailVendedorPedido(AllTrim(ZC5->ZC5_NUMPV)),oFunc:getErro(),ZC5->ZC5_STATUS,.T.,,,ZC5->ZC5_PVVTEX)
				EndIF
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function M008CONPG(cCanal,cPlataforma)
...
cRetorno:=(cAliasQry)->CPDEF
	...
	Else
		cQry += "	SELECT 	SE4.E4_CODIGO "+CRLF
		cQry += "	FROM "+RetSqlName("SE4")+" SE4"+CRLF
		cQry += "	WHERE SE4.E4_FILIAL = '"+xFilial("SE4")+"'"+CRLF
		cQry += "	AND SE4.D_E_L_E_T_ = ' '"+CRLF
		cQry += "	AND SE4.E4_XPARCFA="+AllTrim(Str(ZC5->ZC5_QTDPAR))
		dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQry),cAliasQry,.T.,.T.)
		cRetorno:=(cAliasQry)->E4_CODIGO
