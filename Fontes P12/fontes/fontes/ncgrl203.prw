...
User Function NCGRL203()
Local aSays:={}
Local aButtons:={}
Local cPerg:="NCGRL203"
Local lOk:=.F.
...
AADD( aSays, "Estorno Software de Saída IPI,ICMS e ICMS-ST             " )
...
cCpoQry+="FT_CLIEFOR,FT_LOJA,FT_PRODUTO,B1_GRTRIB, B1_ORIGEM, FT_DIFAL, FT_VFCPDIF, FT_ICMSCOM "
...
If mv_par09==1
	...
	Do While (cAliasQry)->(!Eof())
		...
		cLine+='    <Cell ss:Formula="=RC[-1]*RC[-8]"><Data ss:Type="Number">0</Data></Cell>'+Enter
		...
		cLine+='    <Cell><Data ss:Type="Number">'+RL202Trans((cAliasQry)->FT_DIFAL)+'</Data></Cell>'+Enter
		cLine+='    <Cell><Data ss:Type="Number">'+RL202Trans((cAliasQry)->FT_VFCPDIF)+'</Data></Cell>'+Enter
		cLine+='    <Cell><Data ss:Type="Number">'+RL202Trans((cAliasQry)->FT_ICMSCOM)+'</Data></Cell>'+Enter
		cLine+='   </Row>'+Enter
...
ElseIf mv_par09==2
	...
	Do While (cAliasQry2)->(!Eof())
		...
		Do While ( (cAliasQry2)->(!Eof()) ) .And. (cCFOPAtu == (cAliasQry2)->FT_CFOP)
			nIVA	 := 0
			nIVA	 := (cAliasQry2)->FT_MARGEM//GetExcFisc((cAliasQry2)->FT_ESTADO,(cAliasQry2)->FT_FILIAL, (cAliasQry2)->FT_PRODUTO, "F7_MARGEM",dtos((cAliasQry2)->FT_ENTRADA))
			...
			cLine+='    <Cell><Data ss:Type="Number">'+Alltrim(Str(nIVA))+'</Data></Cell>'+Enter
			cLine+='    <Cell><Data ss:Type="Number">'+cValMidICM+'</Data></Cell>'+Enter
...
ElseIf mv_par09==3
	...
	Do While (cAliasQry)->(!Eof())
		...
		If mv_par09==2  .And. !SF2->F2_EMISSAO >= dDataEmiss
	...
	Do While (cAliasQry2)->(!Eof())
		...
		Do While ( (cAliasQry2)->(!Eof()) ) .And. (cEstAtu == (cAliasQry2)->FT_ESTADO)
			...
			If !SF2->(DbSeek ( (cAliasQry2)->( FT_FILIAL+FT_NFORI+FT_SERORI+FT_CLIEFOR+FT_LOJA)  )  )
				SF2->(DbSeek ( (cAliasQry2)->( FT_FILIAL+FT_NFORI+Padr( Val((cAliasQry2)->FT_SERORI),Len((cAliasQry2)->FT_SERORI))+FT_CLIEFOR+FT_LOJA)  )  )
			EndIf
			
			
			If mv_par09==2  .And. !SF2->F2_EMISSAO >= dDataEmiss
				(cAliasQry2)->(DbSkip());Loop
			EndIf
...
If mv_par09==1
	...
	cLine+='    <Cell ss:StyleID="s78"><Data ss:Type="String">IPI MIDIA</Data></Cell>'+CRLF
	cLine+='   </Row>'+CRLF
...
If mv_par09==1
	cNomArq:="ESTORNO_IPI_SAIDA_"+DtoS(MsDate())+STRTRAN(Time(), ":", "")
ElseIf mv_par09==2
	cNomArq:="ESTORNO_ICM_SAIDA_"+DtoS(MsDate())+STRTRAN(Time(), ":", "")
ElseIf mv_par09==3
	cNomArq:="ESTORNO_ICM_ST_SAIDA_"+DtoS(MsDate())+STRTRAN(Time(), ":", "")		
EndIf
...
Local cQuery:=""    
Local cCFOPIPI	:= GetCtParam("NC_CFOPSIP",;
  								  		"5102|5910|5912|5913|5949|5152|6108|6403|6910|6949|6102",;
										"C",;
										"CFOP de estorno de IPI saida",;
										"CFOP de estorno de IPI saida",;
										"CFOP de estorno de IPI saida",;
										.F. )

Local cCFOPICMSST	:= GetCtParam("NC_CFOPSST",;
  								  		"6403|6404",;
										"C",;
										"CFOP de estorno de ICMS ST saida",;
										"CFOP de estorno de ICMS ST saida",;
										"CFOP de estorno de ICMS ST saida",;
										.F. )
										

Local cEstMun	:= GetCtParam("NC_ESTMUNS",;
  										"RO00106|AC00203|AC00104|AM04062|AM02603|AM03536|AM03569|RR00100|AP00303|AP00600",;
										"C",;
										"Estado e municipio estorno de IPI Saida",;
										"Estado e municipio estorno de IPI Saida",;
										"Estado e municipio estorno de IPI Saida",;
										.F. )
...
cQuery+=" From "+RetSqlName("SFT")+" SFT,"+RetSqlName("SD2")+" SD2,"+RetSqlName("SA1")+" SA1,"+RetSqlName("SB1")+" SB1"+CRLF

cQuery+=" Where SFT.FT_FILIAL Between '"+mv_par01+"' And '"+mv_par02+"'"+CRLF

If mv_par09==1
	cQuery+=" And SFT.FT_CFOP In"+FormatIn(cCFOPIPI, "|")+CRLF
ElseIf mv_par09==2
	cQuery+=" And SFT.FT_VALICM>0 And SFT.FT_ESTADO<>'EX'"+CRLF	
ElseIf mv_par09==3
	cQuery+=" And SFT.FT_CFOP IN"+FormatIn(cCFOPICMSST, "|")+CRLF
	cQuery+=" And SFT.FT_ICMSRET > 0 "
EndIf

cQuery+=" And SD2.D2_FILIAL=SFT.FT_FILIAL"+CRLF
...
cCabec+='  <Table ss:ExpandedColumnCount="51" ss:ExpandedRowCount="99999" x:FullColumns="1"'+Enter
...
cCabec+='    <Cell ss:StyleID="s74"><Data ss:Type="String">DIFAL ICMS</Data></Cell>'+Enter
cCabec+='    <Cell ss:StyleID="s74"><Data ss:Type="String">VALOR FECP</Data></Cell>'+Enter
cCabec+='    <Cell ss:StyleID="s74"><Data ss:Type="String">VLR ICMS COM</Data></Cell>'+Enter
cCabec+='   </Row>    '+Enter
